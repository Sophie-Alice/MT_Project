...
p_apear <- p_apear + labs(
  ...
  subtitle = "MT-Fibroblasts: differences in CMFHM_TGF compared to CMF_TGF",
  color    = "NES",       
) +
theme(
  plot.title.position = "plot",
...
ggplot(go_plot_data, aes(x = GeneRatio_num, y = Description)) + 
  ...
  theme_bw() + 
  labs(
    title = "DEG MT-Fibroblasts CMFHM_TGF vs CMF_TGF: GO BP Enrichment", 
...
p_combined <- p_combined + labs(
  title    = "DEG MT-Fibroblasts CMFHM_TGF vs CMF_TGF: Combined GO BP Enrichment",
...
# Upregulated in Fibroblasts upon Macrophage addition (with TGF-B stimulation)
...
descr <- "actin-mediated cell contraction"
...
if (total_genes > 0) {
  for (i in seq(1, total_genes, by = chunk_size)) {
    ...
    print(SCpubr::do_DotPlot(
      ...
      dot.scale = 10,
      plot.title = "actin-mediated cell contraction: Gene Set Expression Across Conditions",
...
for (i in seq(1, total_genes, by = chunk_size)) {
  ...
  print(raincloud.plots(
    ...
    plot_title    = "Fibroblasts CMFHM_TGF vs CMF_TGF: Gene distribution profiles across MT conditions",
    plot_subtitle = paste0("Geneset = actin-mediated cell contraction - Slice ", slice_num),
...
if (total_genes > 0) {
  for (i in seq(1, total_genes, by = chunk_size)) {
    ...
    print(SCpubr::do_DotPlot(
      ...
      features = genes[i:end_idx],
      dot.scale = 10,
      plot.title = "heart contraction: Gene Set Expression Across Conditions",
      plot.subtitle = paste0("Slice ", slice_num, " (Genes ", i, " to ", end_idx, " of ", total_genes, ")")
    ))
  }
}
...
for (i in seq(1, total_genes, by = chunk_size)) {
  ...
  print(raincloud.plots(
    gene_list     = genes[i:end_idx],
    plot_title    = "Fibroblasts CMFHM_TGF vs CMF_TGF: Gene distribution profiles across MT conditions",
    plot_subtitle = paste0("Geneset = heart contraction - Slice ", slice_num),
    seurat_obj    = Fibroblasts,
    colors        = gradient_colors4
  ))
}
...
# Downregulated in Fibroblasts upon Macrophage addition (with TGF-B stimulation)
