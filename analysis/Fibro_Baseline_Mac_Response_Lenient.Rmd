---
title: "Baseline_Mac_Response Lenient Thresholds"
author: "Sophie.Alice"
date: "2026-07-02"
...
# DEGs of Baseline Macrophage response
...
Degs <- DEG.lists$Baseline_Mac_Response
...
p_apear <- p_apear + labs(
  ...
  subtitle = "MT-Fibroblasts: differences in CMFHM_NC compared to CMF_NC",
  color    = "NES",       
) +
theme(
  plot.title.position = "plot",
...
Note: In thus ORA we ONLY have pathways that have an FDR above 0.05!
...
ggplot(go_plot_data, aes(x = GeneRatio_num, y = Description)) + 
  ...
  theme_bw() + 
  labs(
    title = "DEG MT-Fibroblasts CMFHM_NC vs CMF_NC: GO BP Enrichment", 
...
simp.up.GO.show$NES <- simp.up.GO.show$zScore

p_apear <- enrichmentNetwork(
  enrichment = simp.up.GO.show,
  repelLabels = TRUE,
  drawEllipses = TRUE,
  fontSize     = 3,
  colorBy      = "NES",        
...
simp.down.GO.show$NES <- simp.down.GO.show$zScore


p_apear <- enrichmentNetwork(
  enrichment = simp.down.GO.show,
  repelLabels = TRUE,
  drawEllipses = TRUE,
  fontSize     = 3,
  colorBy      = "NES",        
...
combined.enrichment <- rbind(simp.up.GO.show, simp.down.GO.show)
...
p_combined <- p_combined + labs(
  title    = "DEG MT-Fibroblasts CMFHM_NC vs CMF_NC: Combined GO BP Enrichment",
...
combined.enrichment$negP <- -(combined.enrichment$p.adjust)

# 2. Pass that column to nodeSize
p_combined <- enrichmentNetwork(
  enrichment   = combined.enrichment,
  repelLabels  = TRUE,
  drawEllipses = TRUE,
...
# Scale node size directly by raw p.adjust
p_combined <- p_combined + 
  aes(size = negP) +        
  scale_size_continuous(name = "neg. adj. p-value") +
  labs(
    title    = "DEG MT-Fibroblasts CMFHM_NC vs CMF_NC: Combined GO BP Enrichment",
    subtitle = "Node size = neg p.adjust | Node color = Directional z-Score (NES)",
    color    = "Directional z-Score"
  ) +
  theme(
    plot.title.position = "plot",
...
# Upregulated in Fibroblasts upon Macrophage addition (No TGF-B stimulation)
...
for (i in seq(1, total_genes, by = chunk_size)) {
  ...
  print(raincloud.plots(
    ...
    plot_title    = "Fibroblasts CMFHM_NC vs CMF_NC: Gene distribution profiles across MT conditions",
...
descr <- "regulation of heart contraction"
...
if (total_genes > 0) {
  for (i in seq(1, total_genes, by = chunk_size)) {
    ...
    print(SCpubr::do_DotPlot(
      ...
      features = genes[i:end_idx],
      dot.scale = 10,
      plot.title = "regulation of heart contraction: Gene Set Expression Across Conditions",
      plot.subtitle = paste0("Slice ", slice_num, " (Genes ", i, " to ", end_idx, " of ", total_genes, ")")
    ))
  }
}
...
for (i in seq(1, total_genes, by = chunk_size)) {
  ...
  print(raincloud.plots(
    gene_list     = genes[i:end_idx],
    plot_title    = "Fibroblasts CMFHM_NC vs CMF_NC: Gene distribution profiles across MT conditions",
    plot_subtitle = paste0("Geneset = regulation of heart contraction - Slice ", slice_num),
    seurat_obj    = Fibroblasts,
    colors        = gradient_colors4
  ))
}
...
# Downregulated in Fibroblasts upon Macrophage addition (No TGF-B stimulation)
...
descr <- "cell population proliferation"
...
if (total_genes > 0) {
  for (i in seq(1, total_genes, by = chunk_size)) {
    ...
    print(SCpubr::do_DotPlot(
      ...
      features = genes[i:end_idx],
      dot.scale = 10,
      plot.title = "cell population proliferation: Gene Set Expression Across Conditions",
      plot.subtitle = paste0("Slice ", slice_num, " (Genes ", i, " to ", end_idx, " of ", total_genes, ")")
    ))
  }
}
...
for (i in seq(1, total_genes, by = chunk_size)) {
  ...
  print(raincloud.plots(
    gene_list     = genes[i:end_idx],
    plot_title    = "Fibroblasts CMFHM_NC vs CMF_NC: Gene distribution profiles across MT conditions",
    plot_subtitle = paste0("Geneset = cell population proliferation - Slice ", slice_num),
    seurat_obj    = Fibroblasts,
    colors        = gradient_colors4
  ))
}
