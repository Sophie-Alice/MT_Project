...
# DEGs of Modulated Fibrotic response to TGF-B with Macrophages
 ...
 - CMFHM_TGF vs CMFHM_NC = Effect of TGF-Beta on MT-Cardiomyocytes with Macrophages
...
ggplot(plot_df, aes(reorder(pathway, NES), NES)) +
  ...
  labs(
    ...
    subtitle = "MT-Cardiomyocytes: Pathway changes CMFHM_TGF compared to CMFHM_NC"
  ) +
  theme_bw() +
  theme(
    aspect.ratio    = 1.4, 
...
gsea_sig_df # no sign. terms found!
...
#   subtitle = "MT-Cardiomyocytes: differences in CMFHM_TGF compared to CMFHM_NC",
#   color    = "NES",       
# ) +
# theme(
#   plot.title.position = "plot",
...
ggplot(go_plot_data, aes(x = GeneRatio_num, y = Description)) + 
  ...
  theme_bw() + 
  labs(
    title = "DEG MT-Cardiomyocytes CMFHM_TGF vs CMFHM_NC: GO BP Enrichment", 
...
############################ All signif. Pathways - not enough sing. Pathways
...
p_combined <- p_combined + labs(
  title    = "DEG MT-Cardiomyocytes CMFHM_TGF vs CMFHM_NC: Combined GO BP Enrichment",
...
# Upregulated in Cardiomyocytes upon TGF-Beta stimulation (With Macrophages)
...
descr <- "cellular response to chemical stimulus"
...
if (total_genes > 0) {
  for (i in seq(1, total_genes, by = chunk_size)) {
    ...
    print(SCpubr::do_DotPlot(
      ...
      dot.scale = 10,
      plot.title = "cellular response to chemical stimulus: Gene Set Expression Across Conditions",
...
for (i in seq(1, total_genes, by = chunk_size)) {
  ...
  print(raincloud.plots(
    ...
    plot_title    = "Cardiomyocytes CMFHM_TGF vs. CMFHM_NC: Gene distribution profiles across MT conditions",
    plot_subtitle = paste0("Geneset = cellular response to chemical stimulus - Slice ", slice_num),
...
descr <- "animal organ development"
...
if (total_genes > 0) {
  for (i in seq(1, total_genes, by = chunk_size)) {
    ...
    print(SCpubr::do_DotPlot(
      ...
      features = genes[i:end_idx],
      dot.scale = 10,
      plot.title = "animal organ development: Gene Set Expression Across Conditions",
      plot.subtitle = paste0("Slice ", slice_num, " (Genes ", i, " to ", end_idx, " of ", total_genes, ")")
    ))
  }
}
...
for (i in seq(1, total_genes, by = chunk_size)) {
  ...
  print(raincloud.plots(
    gene_list     = genes[i:end_idx],
    plot_title    = "Cardiomyocytes CMFHM_TGF vs. CMFHM_NC: Gene distribution profiles across MT conditions",
    plot_subtitle = paste0("Geneset = animal organ development - Slice ", slice_num),
    seurat_obj    = Cardiomyocytes,
    colors        = gradient_colors4
  ))
}
