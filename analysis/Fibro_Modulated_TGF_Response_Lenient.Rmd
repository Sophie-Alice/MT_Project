...
if (total_genes > 0) {
  for (i in seq(1, total_genes, by = chunk_size)) {
    ...
    print(SCpubr::do_DotPlot(
      ...
      features = genes[i:end_idx],
      dot.scale = 10,
      plot.title = "cell-matrix adhesion: Gene Set Expression Across Conditions",
      plot.subtitle = paste0("Slice ", slice_num, " (Genes ", i, " to ", end_idx, " of ", total_genes, ")")
    ))
  }
}
...
for (i in seq(1, total_genes, by = chunk_size)) {
  ...
  print(raincloud.plots(
    gene_list     = genes[i:end_idx],
    plot_title    = "Fibroblasts CMFHM_TGF vs. CMFHM_NC: Gene distribution profiles across MT conditions",
    plot_subtitle = paste0("Geneset = cell-matrix adhesion - Slice ", slice_num),
    seurat_obj    = Fibroblasts,
    colors        = gradient_colors4
  ))
}
